...
class _AdminMovieFormScreenState extends State<AdminMovieFormScreen> {
  ...
  Future<void> _save() async {
    ...
    if (_posterCtrl.text.trim().isEmpty || _videoCtrl.text.trim().isEmpty) {
      showToast(context, 'Poster va Video URL kiritilishi shart');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        showToast(context, _isEditing ? 'Kino yangilandi' : 'Kino qo\'shildi');
        Navigator.pop(context);
      }
    } catch (e) {
      showToast(context, 'Xatolik yuz berdi, qayta urinib ko\'ring');
    } finally {
